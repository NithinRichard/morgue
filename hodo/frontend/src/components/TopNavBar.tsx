      ...
      <div className={`nav-right${menuOpen ? " open" : ""}`}> 
        <ul className="nav-list">
          <div className="nav-buttons" style={{ display: 'flex', alignItems: 'center' }}>
            <div className="searchBar" style={{ marginRight: 4,  }}>
              <input
                ref={searchInputRef}
                type="search"
                name="search"
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter' && searchValue.trim()) {
                    navigate(`/bodies?search=${encodeURIComponent(searchValue.trim())}`);
                    setSearchValue("");
                  }
                }}
              />
            </div>
            <li className='nav-list-button'><NavLink to="/" className={({ isActive }) => isActive ? 'active' : ''} style={{fontWeight:400}}>New Sale</NavLink></li>
